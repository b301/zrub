...
static void test_1(void)
{
    ...
    TESTS_CHECK(map->length == 5, "ensure length is 5 after setting 5 items");

    zrub_hashmap_remove(map, "first_name");

    // zrub_hashmap_print_verbose(map);
    TESTS_CHECK(map->length == 4, "ensure length is 4 after removing an item");
...
static void test_2(void)
{
    ...
    TESTS_CHECK(map->capacity == 4, "ensure the hashmap capacity is 4");

    zrub_hashmap_set(map, zrub_string_create("hello", 8), zrub_string_create("world", 8));
    TESTS_CHECK(map->length == 1, "check setting \"hello\": \"world\"");

    // zrub_hashmap_print_verbose(map);

    zrub_hashmap_resize(map, 8);
    TESTS_CHECK(map->capacity == 8, "ensure the hashmap capacity is correctly resized to 8");
