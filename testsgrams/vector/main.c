...
int main()
{
    ...
    /* test 2 */
    {
        ...
        CHECK(cslib_vector_push(vec, cslib_string_create("Hello", 128)) != VECTOR_CAPACITY_REACHED, 
            "test 2: ensuring item is pushed");
        CHECK(cslib_vector_push(vec, cslib_string_create("darkness,", 128)) != VECTOR_CAPACITY_REACHED, 
            "test 2: ensuring item is pushed");
        CHECK(cslib_vector_push(vec, cslib_string_create("my old", 128)) != VECTOR_CAPACITY_REACHED, 
            "test 2: ensuring item is pushed");
        CHECK(cslib_vector_push(vec, cslib_string_create("friend.", 128)) != VECTOR_CAPACITY_REACHED, 
            "test 2: ensuring item is pushed");

        char *should_not_fit = cslib_string_create("talk", 128);
        CHECK(cslib_vector_push(vec, should_not_fit) == VECTOR_CAPACITY_REACHED, "test 2: ensuring vector is at it's capacity");
        ...
        CHECK(cslib_vector_push(vec, cslib_string_create("And this is it.", 128)) != VECTOR_CAPACITY_REACHED, "test 2: ensuring item is pushed");
